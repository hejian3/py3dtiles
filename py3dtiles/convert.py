...
def write_tileset(in_folder, out_folder, octree_metadata, offset, scale, rotation_matrix, include_rgb):
    ...
    root_tileset = Node.to_tileset(
        executor, ''.encode('ascii'), octree_metadata.aabb, octree_metadata.spacing, out_folder, scale
    )
    executor.shutdown()
    ...
    tileset = {
        ...
        'geometricError': np.linalg.norm(
            octree_metadata.aabb[1] - octree_metadata.aabb[0]
        ) / scale[0],
        'root': root_tileset,
    }
...
def zmq_process(activity_graph, srs_out_wkt, srs_in_wkt, node_store, octree_metadata, folder, write_rgb, verbosity):
    ...
    while True:
        ...
        if command[0] == CommandType.READ.value:
            content = pickle.loads(command[1])
            command_type = 1
            ...
            init_reader_fn(
                content['id'],
                content['filename'],
                content['offset_scale'],
                content['portion'],
                skt,
                transformer
            )
        elif command[0] == CommandType.PNTS.value:
            command_type = 3
            pnts_writer.run(skt, command[2], command[1], folder, write_rgb)
        elif command[0] == CommandType.JOBS.value:
            command_type = 2
        ...
        elif command[0] == CommandType.SHUTDOWN.value:
            # ack
            break
        else:
...
def convert(files,
    ...
    zmq_processes = [multiprocessing.Process(
        target=zmq_process,
        args=(
            graph, srs_out_wkt, srs_in_wkt, node_store, octree_metadata, outfolder, rgb, verbose)
    ) for i in range(jobs)]
    ...
    while True:
        ...
        while all_processes_busy or zmq_skt.poll(timeout=0, flags=zmq.POLLIN):
            ...
            start = time.time()
            raw_result = zmq_skt.recv_multipart()

            client_id = raw_result[0]
            result = raw_result[1:]
            return_type = result[0]

            # this need to be flattened,

            if return_type == ResponseType.IDLE.value:
                assert client_id not in zmq_idle_clients  # TODO should be a raise
                ...
                zmq_processes_killed += 1
                all_processes_busy = False
            elif return_type == ResponseType.RESULT.value:
            ...
            elif return_type == ResponseType.SAVE.value:
                content = pickle.loads(result[-1])
                processed_points += content['total']
                points_in_progress -= content['total']
        ...
        while state.to_pnts.input and can_queue_more_jobs(zmq_idle_clients):
            ...
            zmq_send_to_process(zmq_idle_clients, zmq_skt, [CommandType.PNTS.value, node_name, datas])
        ...
        if can_queue_more_jobs(zmq_idle_clients):
            ...
            while can_queue_more_jobs(zmq_idle_clients) and potential:
                ...
                if job_list:
                    zmq_send_to_process(zmq_idle_clients, zmq_skt, [CommandType.JOBS.value] + job_list)
        ...
        while (state.reader.input
            ...
            zmq_send_to_process(
                zmq_idle_clients, zmq_skt, [CommandType.READ.value, pickle.dumps(
                    {
                        'filename': file,
                        'offset_scale': (
                            -avg_min, root_scale, rotation_matrix[:3, :3].T if rotation_matrix is not None else None,
                            infos['color_scale'].get(file)),
                        'portion': portion,
                        'id': _id
                    }
                )]
        ...
        if at_least_one_job_ended:
            ...
            if verbose >= 1:
                ...
                print(
                    '\r{:>6} % in {} sec [est. time left: {} sec]'.format(percent, round(now), round(time_left)),
                    end='', flush=True
                )
    ...
    if graph:
        ...
        dateline.add(
            'progression', values, show_dots=False, secondary=True, stroke_style={'width': 2, 'color': 'black'}
        )
